...
************************************************************************************************************
*/
int go_to_erase_flash(void)
{
    char   *buffer;
    MBR    *mbr;
    int    i;
	int erase_sector_count = 16 * 1024 * 1024 / 512;
    char *data = NULL;
	__u32 tmp_size;
	__u32 tmp_start;

	data = (char *)(0x48000000);
    ...
    for(i=0;i<mbr->PartCount;i++)
    {
        if(!strcmp("sysrecovery", (char *)mbr->array[i].name))
        {
            continue;
        }

		tmp_start = mbr->array[i].addrlo;
		if (mbr->array[i].lenlo > (48 * 1024 * 1024 / 512))
		{
			tmp_size = 48 * 1024 * 1024 / 512; 			//how many sector do you want to erase ?
		}
		else
		{
			tmp_size = mbr->array[i].lenlo;
		}
		__inf("erase sdcard part: %s ,start sector=%d, erase sector count=%d\n",\
        mbr->array[i].name, mbr->array[i].addrlo, tmp_size);
		while(tmp_size >= 16 * 1024 * 1024 / 512)
		{
			wBoot_block_write(tmp_start, 16 * 1024 * 1024 / 512, data);
			tmp_start += 16 * 1024 * 1024 / 512;
			tmp_size  -= 16 * 1024 * 1024 / 512;
		}
    }
