...
//	}
	...
	if(erase_flash)
	{
	  ...
	  int erase_sector_count = 16 * 1024 * 1024 / 512;//how many sector do you want to erase ?
      //data = wBoot_malloc(512*erase_sector_count);
	  data = (char *)(0x48000000);
      //memset(data,0xff,512*erase_sector_count);
	  if (!data) {
		__inf("can not malloc for erase_data\n");
		return -1;
	  }
      memset(data, 0xff, 512*erase_sector_count);
...
*/
			part_start = mbr_info->array[i].addrlo;
			part_size = 48 * 1024 * 1024 / 512; 		//Erase the file system, how large range???

			__inf("erase sdcard part: %s ,start sector=%d,sector count=%d\n",\
		mbr_info->array[i].name,mbr_info->array[i].addrlo, part_size);


			while (part_size >= 16 * 1024 * 1024 / 512) {
		if(SDMMC_LogicalWrite(part_start, 16 * 1024 * 1024 / 512, data, 2))
		{
			__inf("erase sdcard part %s fail!\n",mbr_info->array[i].name);
			wBoot_free(data);
			return -1;
		}
				part_start += 16 * 1024 * 1024 / 512;
				part_size  -= 16 * 1024 * 1024 / 512;
			}
		}
     wBoot_free(data);
	}
